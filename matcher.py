...
from osc_parser.matching.predicates import build_block_query
from osc_parser.matching.executor import match_block
...
for block in top:
    call = block["calls"][0]  # your normalized call
    print(call)

# 4) compile into a query
Q, _pairs = build_block_query(call, fps=10, cfg={
...
all_hits = []
for seg_id, feats in feats_by_seg.items():
    hits = match_block(feats, Q, fps=10)
    for h in hits:
        h["segment"] = seg_id
